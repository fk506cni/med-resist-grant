...
以下は実装完了した Steps 0-7 のプロンプトです。
prompts.md から退避済み。参照用に保持しています。
...
- [x] 様式6のドロップダウンが機能する（テーマ選択等）
- [x] 様式6の総額列に式が維持されている（or 正しい合計値が入っている）
- [x] 様式7の研究者が機関ごとにグループ化されている
- [x] 様式8のPIメールが行3に入っている
- [x] `python -m py_compile main/step03_excel/fill_excel.py` がエラーなし
...
- [x] `./scripts/build.sh clean` が全output/を削除する
- [x] RUNNER=uv に切り替えても動作する
...
- [x] スクリプトが実行可能（chmod +x済み）
- [x] 全ファイルが揃っている場合に正常終了する
- [x] 一部ファイルが欠けている場合にエラーメッセージが表示される
- [x] ファイルサイズが10MBを超える場合に警告が出る
- [x] チェックリストが標準出力に表示される
...
- [x] data/dummy/ に YAML 4ファイルが配置されている
- [x] `./scripts/build.sh validate` がダミーYAMLで通る
- [x] `./scripts/build.sh` がダミーデータで全ステップ完走する（data/source/ が必要なステップはスキップ可）
- [x] 生成された全出力ファイルが開ける
...
# Step 8-9 アーカイブ（2026-04-15 退避）

Step 8 (共同執筆環境) と Step 9 (ナラティブ挿入) は実装完了につき prompts.md から退避。
...
- [x] フルビルドが全ステップ OK
- [x] youshiki1_5_filled.pdf に様式1-2/1-3の本文が含まれている
- [x] テンプレートヘッダが維持されている
- [x] ページ番号が通し番号
- [x] フォント・スタイルが統一
- [ ] 様式1-2 が15ページ以内 ⚠️ LibreOfficeで18p（Word環境での再確認要）
- [x] E2Eテスト通過
- [x] create_package.sh のチェックリストが正しい
