...
- budget:
  - yearly:（年度ごとのリスト、Type Aは最大3年分）
    - year: 1
      items: 物品費（設備備品費 + 消耗品費）
      personnel: 人件費・謝金
      travel: 旅費
      other: その他
    ※ 間接経費は直接経費の30%で自動計算
  - by_institution:（機関ごとの年度別予算内訳）

## 注意
- プレースホルダ値で作成し、各フィールドにYAMLコメントで説明と対応様式を付記
- 予算の indirect_rate は 0.3（30%）固定
- 金額の単位は千円（様式準拠）
```
...
# 8. 概要図
<!-- 最終目標、実施項目、研究者間連携の関係を図示 -->
<!-- ![概要図](figs/overview.svg) or ![概要図](figs/overview.png) -->
...
このスクリプトは上記のうちテーブル記入型の様式にデータを書き込みます。
...
3. 各テーブルを特定し、対応するセルにデータを書き込み:
   - 様式1-1: config.yaml → テーブルセル
   - 様式2-1: config.yaml.budget → 予算テーブル
   - 様式2-2: config.yaml.budget.yearly → 年度別テーブル（×最大5）
...
4. main/step02_docx/output/youshiki1_5_filled.docx に保存
...
- 間接経費（30%）は直接経費から自動計算
- 様式5（法人概要）は大学の場合は不要 — config.yaml のフラグで制御
...
## 機能
- main/step01_narrative/youshiki1_2.md → main/step02_docx/output/youshiki1_2.docx
- main/step01_narrative/youshiki1_3.md → main/step02_docx/output/youshiki1_3.docx
...
- reference-doc: スタイル（フォント、段落書式）を元の様式から継承
  → data/source/r08youshiki1_5.docx をコピーして templates/reference.docx を作るか、
     カスタムreference docxを作成する
  ※ reference-docは「スタイル定義のみ」を参照する。内容は無視される。

## スクリプト構造
1. reference.docxの準備（なければ作成）
...
- [ ] main/step02_docx/output/youshiki1_2.docx が生成される
- [ ] main/step02_docx/output/youshiki1_3.docx が生成される
...
- make package: パッケージング（scripts/package.sh）
...
2. 提出に必要なファイルが揃っているかバリデーション:
   - 必須: youshiki1_5_filled.docx, youshiki1_2.docx, youshiki1_3.docx,
