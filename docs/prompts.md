...
- [ ] **A 案**: 最小例 textbox-table が docx に出力されビルド通過
- [ ] **A 案**: Windows Word PDF / LibreOffice PDF で表示確認（破綻なし）
- [ ] **A 案**: 既存画像 textbox の挙動が変化していない（dummy E2E 通過）
- [ ] **B 案**: lua filter で kind 属性の自動判定が機能
- [ ] **B 案**: wrap_textbox.py で kind に応じた処理分岐が機能
- [ ] **B 案**: 画像のみの textbox で pre-B 案と bit-level diff なし
- [ ] **B 案**: 画像/テーブル混在 textbox でも崩れない
- [ ] **C 案**: table-style 属性で見た目を制御
- [ ] **C 案**: crossref `{#tbl:foo}` が機能
- [ ] **C 案**: テーブル列幅が textbox 幅に自動フィット
- [ ] **C 案**: next-gen-comp-paper レベルの描画品質達成
- [ ] 全案共通: 様式1-2 が 15 ページ以内、docx <10MB
- [ ] 全案共通: inject 後の filled docx で `w:tbl` が `wps:txbx` 内に保持
- [ ] 全案共通: `wp:docPr/@id` の一意性が崩れない
